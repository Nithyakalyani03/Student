using { com.satinfotech.studentdb1 as db} from '../db/schema';
...
annotate studentdb1.Student with @odata.draft.enabled;
annotate studentdb1.Courses with @odata.draft.enabled;
annotate studentdb1.Languages with @odata.draft.enabled;
annotate studentdb1.Books with @odata.draft.enabled;


annotate studentdb1.Student with {
...
annotate studentdb1.Student.Languages with @(
    UI.LineItem:[
        {
            Label: 'Languages',
            Value: lang_ID
...
annotate studentdb1.Courses.Books with @(
    UI.LineItem:[
        {
            Label: 'Books',
            Value: books_ID
...
annotate studentdb1.Languages with @(
    UI.LineItem:[
        {
            Value: code
        },
...
annotate studentdb1.Books with @( 
    UI.LineItem:[
        {
            Value: code
        },
...
annotate studentdb1.Courses with @(
    UI.LineItem:[
        {
            Value: code
        },
...
annotate studentdb1.Gender with @(
    UI.LineItem:[
   {
        @Type: 'UI.DataField',
        Value: code
...
annotate studentdb1.Student with @(
    ...
    UI.SelectionFields: [ first_name , last_name, email_id,pan_no,dob,age,is_alumni],       
